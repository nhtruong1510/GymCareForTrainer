...
-(void)shrinkImage : (float) maxVal{
    ...
    while (((((float)(UIImageJPEGRepresentation(compressedImage, compressionVal).length))/(1048576.000000000f)) > maxVal) && (totalIterations < 1024)) {
        ...
        compressionVal *= 0.7;//subtracts 3% of it's current value just incase above algorithm limits at just above MaxVal and while loop becomes infinite.
        ...
        if ((iterations >= 3) || (compressionVal < 0.1)) {
            ...
            resizeAmount *= 0.7;//3% boost just incase image compression algorithm reaches a limit.
